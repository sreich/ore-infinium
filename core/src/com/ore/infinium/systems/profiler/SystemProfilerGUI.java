...
public class SystemProfilerGUI extends Window {
    ...
    public SystemProfilerGUI(Skin skin, String style) {
        ...
        add(pane).fillX().pad(0, 10, 10, 10).top().prefWidth(MIN_LABEL_WIDTH * 7).minWidth(0);
        pack();
    }
     ...
     *
     * @param renderer
     *         {@link ShapeRenderer} to use, must be ready and set to Line type
     */
    ...
    private static void drawGraphAxis(ShapeRenderer renderer, float x, float y, float width, float height,
                                      float alpha) {
        float sep = height / 7;
        y += sep / 2;
    ...
    private static void graphProfileTimes(ShapeRenderer renderer, float x, float y, float width, float height,
                                          float alpha) {
        ...
        for (SystemProfiler profiler : SystemProfiler.get()) {
            ...
            for (int i = times.length - 1; i >= 1; i--) {
                ...
                // we want do skip line between actaul first and last points, as that may result in ugly line at the
                // edge
                if (current != skip && currentPoint > 0) {
                    renderer.line(x + (i - 1) * sampleLen, y + prevPoint * height / 6, x + i * sampleLen,
